...
- **FastMCP:** test pure helpers (`startup_check`, the `ping` tool function) directly. Don't spin up the server in unit tests.

## Files
| file | covers |
|------|--------|
| `test_config.py` | Settings hard-fail + happy path |
| `test_sat_client.py` | full error-mapping table, auth headers, healthz |
| `test_stations.py` | Atlas Search pipeline shape + projection + limit |
| `test_server.py` | startup_check fail-fast behavior, ping tool |
